...
class Multicast extends utils.Adapter {
  ...
  async onReady() {
    ...
    this.log.info(
      "Starting Multicast Adapter - Version 0.2.0 from 7. August 2023",
    );
    receive_ip = String(
      `${this.config.receive_1}.${this.config.receive_2}.${
        this.config.receive_3
      }.${this.config.receive_4}`,
    );
    receive_port = Number(this.config.receive_port);
    send_ip = String(
      `${this.config.send_1}.${this.config.send_2}.${this.config.send_3}.${
        this.config.send_4
      }`,
    );
    ...
    this.log.debug(
      `Retrymax count read : ${
        retrymaxcount
      } as type : ${typeof retrymaxcount}`,
    );
    this.log.debug(
      `Retrytime read : ${retry_time} as type : ${typeof retry_time}`,
    );
    ...
    multicast.on("listening", () => {
      ...
      this.log.info(
        `Multicast Client listening on ${receiveaddress.address}:${
          receiveaddress.port
        } and send to ${send_ip}:${send_port}`,
      );
    });
    ...
    multicast.on("message", (message, remote) => {
      ...
      try {
        ...
        if (received_data.Type === "Object") {
          // initialization
          this.log.debug(
            `Object-Data from device ${JSON.stringify(received_data)}`,
          );
        ...
        this.log.error(
          `unknown message received - please read the documentation !!!! - this was received: ${
            message
          }`,
        );
      }
  ...
  async onStateChange(id, state) {
    if (state) {
      ...
      if (state.ack === false) {
        ...
        let object;
        if (
          !existingDevices[deviceId[2]] ||
          !existingDevices[deviceId[2]].common
        ) {
          this.log.debug(
            `No common data cached for ${deviceId[2]}, loading common object`,
          );
          object = await this.getObjectAsync(deviceId[2]);
        } else {
          this.log.debug(
            `Common data cache for ${deviceId[2]} | ${JSON.stringify(existingDevices[deviceId[2]])}`,
          );
        ...
        if (object !== undefined && object !== null) {
          this.log.debug(
            `AsyncObject received while StateChange: ${JSON.stringify(object)}`,
          );

          // ToDo: rebuild to handle retry logic correctly
          if (
            stateSendCache[deviceId[2]] == null ||
            stateNameToSend[deviceId[2]].s == null
          ) {
          ...
          if (stateSendCache[deviceId[2]].timer) {
            clearTimeout(stateSendCache[deviceId[2]].timer);
            stateSendCache[deviceId[2]].timer = null;
          }
  ...
  async DoResetConnState() {
    ...
    for (const i in device_list) {
      ...
      this.log.debug(
        `${JSON.stringify(
          device_list[i].common.Devicename,
        )} Connection state set to FALSE`,
      );
      //@ts-ignore : Devicename exist in Device Object, custom attribute
      this.setState(`${device_list[i].common.Devicename}.Info.Connected`, {
        val: false,
        ack: true,
        expire: 70,
      });
  ...
  async DoInfoStates(received_data, device, create) {
    try {
      ...
      this.log.debug(
        `Return of async getObject at Info channel creation: ${JSON.stringify(
          device_cur,
        )}`,
      );
      // In case of device not found, request complete device
      if (
        (device_cur === undefined || device_cur === null) &&
        create === false
      ) {
        ...
        for (const i in objects) {
          ...
          this.log.debug(
            `Contain of array for info in device loop : ${JSON.stringify(
              array,
            )}`,
          );
          ...
          if (create === true) {
            await this.DoStateCreate(
              `${device}.Info.${objects[i]}`,
              objects[i],
              "string",
              received_data.i[objects[i]].r,
              "",
              writable,
              min,
              max,
          ...
          if (typeof received_data.i[objects[i]] === "object") {
            await this.setStateAsync(`${device}.Info.${objects[i]}`, {
              val: received_data.i[objects[i]].val,
              ack: true,
              expire: received_data.i[objects[i]].expire,
            });
          } else {
            await this.setStateAsync(`${device}.Info.${objects[i]}`, {
              val: received_data.i[objects[i]],
              ack: true,
            });
          }
          object.common = array;
  ...
  async DoInitialize(received_data, device) {
    ...
    await this.setObjectNotExistsAsync(
      `${received_data.i["Devicename"]}.Info.Connected`,
      {
        type: "state",
        common: {
          name: "online",
    ...
    await this.setStateAsync(
      `${received_data.i["Devicename"]}.Info.Connected`,
      { val: true, ack: true, expire: 70 },
    );
    // Create info channels and write values in object
    this.log.debug(
      `Create info changes with data : ${JSON.stringify(
        received_data,
    ...
    try {
      ...
      for (const i in config) {
        ...
        if (received_data.c[config[i]].c === undefined) {
          ...
          stateCreateName = `${device}.Config.${received_data.c[config[i]].c}.${
            config[i]
          }`;
        }
        ...
        await this.DoStateCreate(
          stateCreateName,
          config[i],
          received_data.c[config[i]].t,
          received_data.c[config[i]].r,
          received_data.c[config[i]].u,
          writable,
          min,
          max,
        );
        await this.setStateAsync(stateCreateName, {
          val: received_data.c[config[i]].v,
          ack: true,
        });
    ...
    try {
      ...
      for (const i in StringifiedStateNames) {
        ...
        await this.DoStateCreate(
          stateCreateName,
          splittedDevice[splittedDevice.length - 1],
          StringifiedStateNames[i].t,
          StringifiedStateNames[i].r,
          StringifiedStateNames[i].u,
          writable,
          null,
          null,
          comm_states,
        ...
        this.log.debug(
          `Value for in state creation : ${StringifiedStateNames[i].v}`,
        );
        if (
          StringifiedStateNames[i].e !== undefined &&
          StringifiedStateNames[i].e !== null
        ) {
          this.log.debug(
            `State ${stateCreateName} with value ${
              StringifiedStateNames[i].v
            } and expire ${StringifiedStateNames[i].e}`,
          );
          await this.setStateAsync(stateCreateName, {
            val: StringifiedStateNames[i].v,
            ack: true,
            expire: StringifiedStateNames[i].e,
          });
        } else {
          this.log.debug(
            `State ${stateCreateName} with value ${StringifiedStateNames[i].v}`,
          );
          await this.setStateAsync(stateCreateName, {
            val: StringifiedStateNames[i].v,
            ack: true,
  ...
  async DoStateupdate(received_data, confirm) {
    ...
    if (object === undefined || object === null) {
      ...
      for (const i in stateNames) {
        ...
        this.log.debug(
          `Statename to update : ${statename} with value : ${
            received_data.s[stateNames[i]]
          }`,
        );
        const deviceId = statename.split(".");
        this.log.debug(`Statename to send : ${deviceId[2]}`);
        this.log.debug(
          `Type of state : ${typeof received_data.s[stateNames[i]]}`,
        );
  ...
  DoTimesync() {
    ...
    if (this.config.Time_Sync !== undefined && this.config.Time_Sync !== null) {
      ...
      this.log.error(
        "Error reading time-interval settings, using value of 1 minute",
      );
      interval = 60000;
    }
  ...
  async DoStateRestore(received_data) {
    const device = received_data.i["Devicename"];
    // Set connected flag
    this.setState(`${received_data.i["Devicename"]}.Info.Connected`, {
      val: true,
      ack: true,
      expire: 70,
    });
    ...
    if (object === undefined || object === null) {
      ...
      for (const i in states) {
        ...
        if (deviceId[3] !== "Info") {
          ...
          this.log.debug(
            `Received data from getstate in recovery function : ${JSON.stringify(
              value,
            )}`,
          );
          if (value !== null && value !== undefined) {
            this.log.debug(
              `Statename to recover : ${stateNameToSend} with value : ${
                value.val
              }`,
            );
  ...
  async DoHeartbeat(received_data) {
    ...
    const object = await this.getObjectAsync(received_data.i["Devicename"]);
    this.log.debug(
      `Device object data in Heartbeat: ${JSON.stringify(object)}`,
    );
    if (object === undefined || object === null) {
      this.DoGetDevice(received_data);
    } else {
      ...
      (function () {
        if (timeout_connection[received_data.i["Devicename"]]) {
          clearTimeout(timeout_connection[received_data.i["Devicename"]]);
          timeout_connection[received_data.i["Devicename"]] = null;
        }
      })();
  ...
  async DoCheckConnection(received_data) {
    ...
    const con_state = await this.getStateAsync(
      `${received_data.i["Devicename"]}.Info.Connected`,
    );
    if (con_state != null) {
  ...
  DoTransmit(message, msgID) {
    ...
    if (
      messagesRetryCache[msgID].ack === false &&
      messagesRetryCache[msgID].count < retrymaxcount + 1
    ) {
      ...
      multicast.send(
        ...
        (err, bytes) => {
          ...
          this.log.debug(
            `Multicast Message to IP ${send_ip} on Port ${parseInt(
              send_port,
            )} transmitted! Length of message was ${message.length} with ${
              bytes
            } bytes `,
          );
        },
      ...
      // Buffer messages before sending to combine mu
      if (messagesRetryCache[msgID].timer) {
        clearTimeout(messagesRetryCache[msgID].timer);
        messagesRetryCache[msgID].timer = null;
      }
      ...
      this.log.error(
        `Device ${message.common.id} not response to message ${msgID}`,
      );
      this.setState(`${message.i["Devicename"]}.Info.Connected`, {
        val: false,
        ack: true,
      });
    }
