...
class Multicast extends utils.Adapter {
    ...
    async onReady() {
        ...
        receive_ip = String(
            `${this.config.receive_1}.${this.config.receive_2}.${this.config.receive_3}.${this.config.receive_4}`,
        );
        ...
        multicast.on('message', (message, remote) => {
            ...
            try {
                ...
                this.log.error(
                    `unknown message received - please read the documentation !!!! - this was received: ${message}`,
                );
            }
        });
    }
    ...
    onUnload(callback) {
        try {
            ...
            if (timeSyncInterval) {
                this.clearInterval(timeSyncInterval);
                timeSyncInterval = null;
            }
            multicast.close();
            callback();
            ...
            if (state.ack === false) {
                ...
                if (object !== undefined && object !== null) {
                    ...
                    if (stateSendCache[deviceId[2]].timer) {
                        this.clearTimeout(stateSendCache[deviceId[2]].timer);
                        stateSendCache[deviceId[2]].timer = null;
                    }
                    stateSendCache[deviceId[2]].timer = this.setTimeout(() => {
                        // Send message
    ...
    async DoInitialize(received_data, device) {
        ...
        await this.setStateAsync(`${received_data.i['Devicename']}.Info.Connected`, {
            val: true,
            ack: true,
            expire: 70,
        });
    ...
    DoTimesync() {
        ...
        timeSyncInterval = this.setInterval(() => {
    ...
    async DoHeartbeat(received_data) {
        ...
        if (object === undefined || object === null) {
            ...
            if (timeout_connection[received_data.i['Devicename']]) {
                this.clearTimeout(timeout_connection[received_data.i['Devicename']]);
                timeout_connection[received_data.i['Devicename']] = null;
            }
            timeout_connection[received_data.i['Devicename']] = this.setTimeout(() => {
                this.setState(`${received_data.i['Devicename']}.Info.Connected`, {
                    val: false,
                    ack: true,
                });
            ...
            multicast.send(message, 0, message.length, parseInt(send_port), send_ip, (err, bytes) => {
                ...
                this.log.debug(
                    `Multicast Message to IP ${send_ip} on Port ${parseInt(
                        send_port,
                    )} transmitted! Length of message was ${message.length} with ${bytes} bytes `,
                );
            });

            // Buffer messages before sending to combine mu
            if (messagesRetryCache[msgID].timer) {
                this.clearTimeout(messagesRetryCache[msgID].timer);
                messagesRetryCache[msgID].timer = null;
            }
            messagesRetryCache[msgID].timer = this.setTimeout(() => {
                // Send message
